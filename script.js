...
const descbg = document.getElementById('descriptionbackground')
let inDesc = false;
        ...
        if (boxclickstatus[clicked]===false){
            if (alive===true){
                if (boxbombstatus[bomb]===false){
                    ...
                    // put the math here
                    surviveChance = 1;
                    for(let i=0; i<=boxClicked-1; i++){
                        surviveChance *= (25-(minesamount+i))/(25-i);
                    }
                    dyingChance = 1-surviveChance;
                    multiplier = (1+(0.5*dyingChance - 0.2))/(-1*dyingChance + 1);
                    if (multiplier<1){
                        multiplier-=0.1
                    }
...
function opendesc(){
    if (inDesc===false){
        ...
        descbg.style.display = 'block';
        inDesc= true;
    } else {
        desc.style.display = 'none';
        descbg.style.display = 'none';
        inDesc=false;
    }
}
